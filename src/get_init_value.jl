...
function _get_binary_status_for_ED(uc_model::JuMP.Model, thermal_gen_names; CoverHour)
    ug_t0 = Dict(g => [Int(value(uc_model[:ug][g,t])) for t in 1:CoverHour] for g in thermal_gen_names)
    vg_t0 = Dict(g => [Int(value(uc_model[:vg][g,t])) for t in 1:CoverHour] for g in thermal_gen_names)
    wg_t0 = Dict(g => [Int(value(uc_model[:wg][g,t])) for t in 1:CoverHour] for g in thermal_gen_names)
