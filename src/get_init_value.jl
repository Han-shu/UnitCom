...
function _init_fr_ed_model(sys::System; theta::Union{Nothing, Int64} = nothing)
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer, VOLL; uc_op_price = OrderedDict("BA" => [0,0],"PH" => [0,0]), theta = theta, start_time = DateTime(2018,12,31,21))
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer, VOLL; uc_op_price = OrderedDict("BA" => [0,0],"PH" => [0,0]), init_value = ED_init_value, theta = theta, start_time = DateTime(2018,12,31,21))
