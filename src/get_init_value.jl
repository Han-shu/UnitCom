...
function _init_fr_ed_model(sys::System; theta::Union{Nothing, Int64} = nothing)
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer; uc_LMP = [0,0], theta = theta, start_time = DateTime(Date(2019, 1, 1)))
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer; uc_LMP =[0,0], init_value = ED_init_value, theta = theta, start_time = DateTime(Date(2019, 1, 1)))
