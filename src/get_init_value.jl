...
function _init_fr_ed_model(sys::System; scenario_cnt, horizon, theta::Union{Nothing, Int64} = nothing)
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer, VOLL; scenario_count = scenario_cnt, horizon = horizon, storage_value = OrderedDict("BA" => 35,"PH" => 35), theta = theta, start_time = DateTime(2018,12,31,21))
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer, VOLL; horizon = horizon, scenario_count = scenario_cnt, storage_value = OrderedDict("BA" => 35,"PH" => 35), init_value = ED_init_value, theta = theta, start_time = DateTime(2018,12,31,21))
