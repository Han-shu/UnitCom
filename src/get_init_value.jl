...
function _init_fr_ed_model(sys::System; scenario_cnt, horizon, theta::Union{Nothing, Int64} = nothing)
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer, VOLL; scenario_count = scenario_cnt, horizon = horizon, uc_op_price = OrderedDict("BA" => [0,0],"PH" => [0,0]), theta = theta, start_time = DateTime(2018,12,31,21))
    ...
    model = stochastic_ed(sys, Gurobi.Optimizer, VOLL; horizon = horizon, scenario_count = scenario_cnt, uc_op_price = OrderedDict("BA" => [0,0],"PH" => [0,0]), init_value = ED_init_value, theta = theta, start_time = DateTime(2018,12,31,21))
