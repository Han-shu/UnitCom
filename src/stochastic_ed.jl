...
function ed_model(sys::System, optimizer; theta = nothing, VOLL = 1000, start_time = DateTime(Date(2019, 1, 1)))
    ...
    if isnothing(theta)
        ...
        for g in solar_gens
            net_load -= get_time_series_values(Scenarios, g, "solar_power", start_time = start_time, len = length(time_periods))[:, theta]
        end
        for g in wind_gens
            net_load -= get_time_series_values(Scenarios, g, "wind_power", start_time = start_time, len = length(time_periods))[:, theta]
        end
        for load in get_components(StaticLoad, sys)
            net_load += get_time_series_values(Scenarios, load, "load", start_time = start_time, len = length(time_periods))[:, 100-theta]
        end
    end
